...
| database   | Repository portları, Supabase erişimi ve atomik SQL RPC'leri                     | domain, ingestion                      |
...
| web        | Next.js App Router iskeleti                                                      | domain, validation                     |
...
`published_at`, `verified_at`, `effective_at`, `[valid_from, valid_until)`, `superseded_at`, `reference_period` ayrı kavramlardır. `validateFactPublication` dönem ve geçerlilik; `validateCurrentEvidence` current version, son gözlem, son başarılı kontrol ve kaynak sağlığı önkoşullarını sağlar. Prompt 3 yayın/sorgu katmanı iki kontrolü birlikte zorunlu uygulamalıdır. Eski yılın fact'i güncel yılın sorusuna taşınamaz.
...
Bir kurum, sayfa veya PDF hatası sonraki kurumları durdurmaz. Yarım kalmış process için kalıcı lease/recovery ve çoklu worker'lar arası ortak rate limit henüz yoktur; üretim işletimi tek worker ve haricî zamanlayıcı gerektirir.
