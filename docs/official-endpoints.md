...
| ÖSYM     | [YKS duyuru arşivi](https://www.osym.gov.tr/SinavGrubu/Menu/314) | Aynı ÖSYM adapter'ı; YKS grubundaki resmî başlıklar ve ayrıntı sayfaları                                | Genel listeden düşmüş 2026 YKS sonuç duyuruları bu kayıtlı YKS penceresinden yeniden bulunur. İlk 50 belge ile sınırlıdır; tüm tarihsel arşiv kapsamı iddiası yoktur. |
...
- ÖSYM'nin [2026-YKS yerleştirme sonuç duyurusu](https://www.osym.gov.tr/2026-yks-yerlestirme-sonuclari-aciklandi), ek sayısal bilgi bağlantısını `cdn.osym.gov.tr` üzerinden verir. Bu doğrudan resmî ek host'u yalnız bu gözlemden sonra allowlist'e eklendi.
...
Beş kurum aktiftir. Eski çıplak `osym.gov.tr`, `meb.gov.tr`, `yok.gov.tr`, `gsb.gov.tr`, `yokak.gov.tr` kayıtları **candidate** kalır; `include_subdomains=false`. Etkin ÖSYM host'ları `www.osym.gov.tr`, `dokuman.osym.gov.tr` ve doğrulanmış ek host `cdn.osym.gov.tr` ile sınırlıdır. Yeni bulunan bir alt alan adı kendiliğinden etkinleştirilmez.
