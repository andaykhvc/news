...
- Supabase repository'leri, migration, seed üretimi ve default-deny RLS/izinler.
...
Uygun framework/servis sayısı küçük tutuldu: Next.js + PostgreSQL/Supabase + Node worker. Redis, vector DB, LLM, ayrı API sunucusu veya production scheduler yoktur.
...
Çalışan veri alımı ilk dokuz tabloyu kullanır. Kalan modeller ileride sınıflandırma/doğrulama/yanıt desteği içindir. Tüm tabloların RLS ve izinleri denetlendi; halka açık okuma/yazma açılmadı. Migration yerel Docker Supabase PostgreSQL 17 üzerinde uygulandı.
...
- PostgreSQL/PGlite: migration, 17 RLS tablosu, atomiklik/rollback, idempotency, stale fetch, immutable history, composite FK, rol yetkileri, evidence gereksinimi ve topic cycle kontrolü.

Ek olarak canlı yerel Supabase PostgREST üzerinden repository okuması yapıldı: 5 kurum, 5 host, 0 endpoint. Geçersiz ingestion RPC reddedildi ve anon rolünün tablo okuması engellendi. Next.js üretim sunucusunda `/` ve `/health` 200 döndürdü.
...
| `supabase start` (gereksiz hizmetler dışlanarak)                           | Docker üzerinde başarılı                                 |
| `supabase db reset --local`                                                | Bu projeye ait yerel DB'de migration + seed başarılı     |
| `supabase db lint --local --schema public --fail-on warning`               | Uyarı/hata yok                                           |
| `supabase db advisors --local --type security --level warn --fail-on warn` | Sorun yok                                                |
| `supabase migration list --local`                                          | Migration uygulanmış                                     |
...
Gerçek kurum taraması, otomatik haber yayını, LLM veya production cron bu teslimde çalışır durumda değildir. Uzaktaki bir Supabase projesine veya canlı siteye deployment yapılmadı.
